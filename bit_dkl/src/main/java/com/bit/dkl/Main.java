...
//        LocalPredictor wcyPredictor = new LocalPredictor(wcyPath, wcyInputDataSchema);
        ...
        LocalPredictor dklPredictor = new LocalPredictor(dklPath, dklInputDataSchema);

        //Gui.runGui(dklPredictor, "--linux");

        if (args.length == 1 && args[0].equals("--gui")) {
            Gui.runGui(dklPredictor, "--linux");
        } else   if (args.length == 2 && args[0].equals("--gui")) {
            Gui.runGui(dklPredictor, args[1]);
        } else {
            Terminal.runTerminal(dklPredictor);
        }
    }
}
